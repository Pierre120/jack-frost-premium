<script lang="ts">
	...
	const isResetError = (errMsg: string | undefined) => {
		if (errMsg) {
			return true;
		}
		return false;
	...
	$: resetError = isResetError(form?.message);
	$: adminEmail = form?.data?.admin_email;
</script>

<svelte:head>
	<title>Admin Password Reset | Jack Frost Ice Cream Premium</title>
</svelte:head>
		...
		>
			<div class="p-6 space-y-4 md:space-y-6 sm:p-8">
				<form method="POST" use:enhance>
					...
					<!-- Alert for failed Password Reset -->
					{#if form?.message && resetError}
						<div class="pb-4">
