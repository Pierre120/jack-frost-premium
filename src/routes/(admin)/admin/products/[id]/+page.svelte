<script lang="ts">
  ...
  const discardChange = () => {
    ...
    confirmationDetails = "The changes you've made will not be saved if you leave this page without saving.";
...
<EditProductForm label="Edit Product" submitHandle={submitEdit} formaction="?/edit" categories={data.categories} product={data.product} on:close={discardChange} on:remove={deleteProduct}/>
{#if isAboutToLeave || isAboutToDelete}
	<ConfirmationModal {confirmationHeader} {confirmationDetails} {cancelLabel} {confirmLabel} on:cancel={cancel} on:confirm={confirm} />
{/if}
{#if success || deleted}
