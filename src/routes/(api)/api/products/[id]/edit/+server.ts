...
export const POST = (async ({ params, request }) => {
  try {
    ...
    if(img_path.split('/')[0] !== 'products') {
      ...
      const { data: data1, error: error1 } = await supabase.storage.from('images').remove([product.img_path]);
      if (error1) {
				throw error1;
			}
			if (data1) {
      ...
      const { data: data2, error: error2 } = await supabase.storage.from('images').move(img_path, `products/${img_path}`);
			if (error2) {
				throw error2;
			}
			if (data2) {
